  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Dashboard Kerja Teknisi</h1>
          <p className="text-slate-600 text-sm">Kelola tugas servis, dokumentasi pengerjaan, dan pencatatan pembayaran di lokasi.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={fetchJobs}
            className="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-100 text-xs font-medium transition-colors"
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {displayedJobs.map((job) => (
            <div key={job.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between gap-6 hover:border-blue-200 transition-colors">
              <div className="space-y-3 flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-bold text-lg text-slate-800">{job.customer}</span>
                ...
                {job.notes && (
                  <div className="bg-slate-50 p-2.5 rounded-lg text-xs text-slate-600 border border-slate-100">
                    <span className="font-semibold text-slate-700">Catatan Pelanggan:</span> {job.notes}
                  </div>
                )}

                {/* Info Biaya Jika Selesai */}
                {job.status === 'Selesai' && (
                  <div className="p-3 bg-slate-50 rounded-xl text-xs space-y-1 border border-slate-100 text-slate-600">
                    <div className="flex justify-between">
                      <span>Jasa Pokok ({job.service}):</span>
                      <span className="font-semibold text-slate-800">Rp {job.servicePrice.toLocaleString('id-ID')}</span>
                    </div>
                    ...
                    <div className="flex justify-between font-bold text-slate-800 pt-1 border-t border-slate-200">
                      ...
                      <span className="text-blue-700">Rp {job.paymentAmount.toLocaleString('id-ID')}</span>
                    </div>
                  </div>
                )}
              </div>

              <div className="flex flex-col justify-center border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 min-w-[200px] gap-2.5">
                ...
                {job.status === 'Selesai' && (
                  <Link 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-blue-50 text-blue-700 hover:bg-blue-100 font-bold py-2.5 rounded-xl text-xs transition-colors border border-blue-200"
                  >
                    <FileText size={15} />
                    Buka Invoice
                  </Link>
      ...
      {showModal && selectedJob && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto animate-scaleUp">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 sticky top-0 z-10">
              <div>
                <h3 className="font-bold text-lg text-slate-800">Selesaikan Pekerjaan & Catat Pembayaran</h3>
                <p className="text-xs text-slate-500">{selectedJob.requestCode} - {selectedJob.customer}</p>
              </div>
            ...
            <form onSubmit={handleSaveCompletion}>
              <div className="p-6 space-y-5">
                {/* Info Singkat Unit */}
                <div className="bg-blue-50 text-blue-900 p-3.5 rounded-xl text-sm border border-blue-100">
                  ...
                  <p className="text-xs text-blue-700 mt-1">Unit: {selectedJob.acUnit} • {selectedJob.address}</p>
                </div>

                {/* 1. Dokumentasi Foto */}
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">1. Dokumentasi Hasil Servis</h4>
                    ...
                    <div>
                      ...
                      {beforePhoto ? (
                        ...
                        <div 
                          ...
                          className="border-2 border-dashed border-slate-200 hover:border-blue-400 rounded-xl p-4 flex flex-col items-center justify-center text-slate-500 hover:bg-blue-50/40 cursor-pointer text-center"
                        >
                          <Upload size={18} className="mb-1 text-slate-400" />
                          <span className="text-xs font-medium text-slate-700">Foto Kondisi Awal</span>
                    ...
                    <div>
                      ...
                      {afterPhoto ? (
                        ...
                        <div 
                          ...
                          className="border-2 border-dashed border-slate-200 hover:border-emerald-400 rounded-xl p-4 flex flex-col items-center justify-center text-slate-500 hover:bg-emerald-50/40 cursor-pointer text-center"
                        >
                          <Upload size={18} className="mb-1 text-slate-400" />
                          <span className="text-xs font-medium text-slate-700">Foto Hasil Servis</span>
                          <span className="text-[10px] text-slate-400">JPG/PNG</span>
                ...
                <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    <div>
                      <label className="block text-slate-500 font-medium mb-1">Tarif Layanan Dasar</label>
                      <input 
                        type="text" 
                        readOnly 
                        value={`Rp ${selectedJob.servicePrice.toLocaleString('id-ID')}`} 
                        className="w-full bg-slate-100 border border-slate-200 text-slate-700 px-3 py-2 rounded-lg font-semibold"
                      />
                    </div>
                    <div>
                      <label className="block text-slate-500 font-medium mb-1">Biaya Tambahan (Freon / Part)</label>
                      <div className="relative">
                        ...
                        <input 
                          ...
                          placeholder="0"
                          className="w-full pl-9 pr-3 py-2 bg-white border border-slate-300 rounded-lg font-semibold text-slate-800 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  {additionalCost > 0 && (
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs text-slate-800 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      />
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {paymentMethod === 'Tunai (Cash)' && <Check size={16} className="text-emerald-600" />}
                      </div>
                      <span className="text-[11px] text-slate-500 mt-1">Diterima langsung di lokasi</span>
                    </button>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {paymentMethod === 'Transfer Bank' && <Check size={16} className="text-blue-600" />}
                      </div>
                      <span className="text-[11px] text-slate-500 mt-1">Via M-Banking / Rekening</span>
                    </button>
                  </div>
              ...
              <div className="p-6 border-t border-slate-100 flex gap-3 bg-slate-50 sticky bottom-0 z-10">
                <button 
                  type="button"
                  onClick={() => setShowModal(false)} 
                  className="flex-1 bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 font-semibold py-2.5 rounded-xl transition-colors text-xs"
                >
                  Batal
                </button>
                <button 
