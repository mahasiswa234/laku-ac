  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Manajemen Teknisi</h1>
          <p className="text-slate-600 dark:text-slate-400">Kelola data teknisi dan ketersediaan mereka.</p>
        </div>
              ...
              {isLoading ? (
                ...
                filtered.map((tech, i) => (
                  <tr key={tech.id || i} className="hover:bg-slate-50 dark:hover:bg-slate-950">
                    <td className="px-6 py-4 font-medium text-slate-700 dark:text-slate-300">{tech.id}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                        tech.status === 'Aktif' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 
                        tech.status === 'Sibuk' ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300' : 'bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300'
                      }`}>
                        {tech.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => openEditModal(tech)} className="text-blue-500 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 p-1">
                          <Edit2 size={18} />
                        </button>
                        <button onClick={() => handleDelete(tech.id)} className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 p-1">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
