  ...
  return (
    <div className="space-y-6">
      ...
      {activeToast && (
        <div 
          id="live-order-toast"
          className="fixed top-5 right-4 sm:right-6 z-50 max-w-sm sm:max-w-md w-full bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-blue-200/90 p-4 transition-all duration-300 animate-in slide-in-from-top-4"
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-1">
                <h4 className="text-sm font-bold text-slate-900 truncate">{activeToast.title}</h4>
              ...
              <div className="mt-2.5 flex items-center justify-between gap-2 pt-2 border-t border-slate-100 text-xs">
      ...
      {successBanner && (
        <div id="customer-success-banner" className="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-xl flex items-center justify-between shadow-sm animate-fadeIn">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 sm:p-8 text-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative">
        <div className="flex-1">
          <div className="inline-flex items-center gap-2 bg-white/20 px-3 py-1 rounded-full text-xs font-medium mb-2 backdrop-blur-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative" ref={notificationDropdownRef}>
            <button
              ...
              className="relative p-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-colors border border-white/20 flex items-center justify-center"
            ...
            {isNotificationOpen && (
              <div 
                ...
                className="absolute right-0 sm:right-0 top-12 mt-2 w-80 sm:w-96 bg-white text-slate-800 rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150"
              >
                {/* Header Dropdown */}
                <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
                  <div className="flex items-center gap-2">
                    <Bell size={18} className="text-blue-600" />
                    <h3 className="font-bold text-sm text-slate-800">Notifikasi Pesanan</h3>
                    {notifications.filter(n => !n.read).length > 0 && (
                  ...
                  <div className="flex items-center gap-1.5">
                    {notifications.some(n => !n.read) && (
                      <button
                        ...
                        className="text-[11px] font-semibold text-blue-600 hover:text-blue-800 flex items-center gap-1 hover:bg-blue-50 px-2 py-1 rounded-lg transition-colors"
                    ...
                    {notifications.length > 0 && (
                      <button
                        ...
                        className="text-[11px] text-slate-400 hover:text-rose-600 hover:bg-rose-50 p-1.5 rounded-lg transition-colors"
                ...
                <div className="max-h-[360px] overflow-y-auto divide-y divide-slate-100">
                  {notifications.length === 0 ? (
                    <div className="py-10 px-4 text-center">
                      ...
                      <p className="text-xs font-semibold text-slate-600">Belum Ada Notifikasi Baru</p>
          ...
          <button 
            ...
            className="flex-1 sm:flex-initial flex items-center justify-center gap-2 bg-white text-blue-700 hover:bg-blue-50 px-4 py-2.5 rounded-xl font-semibold text-sm transition-all shadow-sm"
          >
            <Plus size={18} />
            Tambah Unit AC
          </button>
          <button 
            ...
            className="flex-1 sm:flex-initial flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-400 text-white px-4 py-2.5 rounded-xl font-semibold text-sm transition-all shadow-sm border border-blue-400/50"
          >
            <Calendar size={18} />
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[1, 2, 3].map(item => (
            <div key={item} className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm animate-pulse flex items-center justify-between">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Unit AC</p>
              <p className="text-2xl font-bold text-slate-800 mt-1">{units.length} Unit</p>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-amber-600 uppercase tracking-wider">Menunggu Admin</p>
              <p className="text-2xl font-bold text-amber-600 mt-1">{pendingCount} Pesanan</p>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wider">Servis Selesai</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">{completedCount} Selesai</p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col h-full">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-800">Unit AC Anda</h2>
                <p className="text-xs text-slate-500">Daftar AC yang terhubung ke akun Anda</p>
              </div>
            </div>
            <button
              onClick={() => setIsUnitModalOpen(true)}
              className="text-xs font-semibold text-blue-600 hover:text-blue-800 flex items-center gap-1 bg-blue-50 px-2.5 py-1.5 rounded-lg transition-colors"
            >
          ...
          {isLoading ? (
            <div className="space-y-3 flex-1">
              {[1, 2].map(i => (
                <div key={i} className="p-4 rounded-xl border border-slate-100 bg-slate-50 animate-pulse space-y-2.5">
                  <div className="flex justify-between items-center">
                    <div className="h-4 w-32 bg-slate-200 rounded"></div>
                    <div className="h-5 w-20 bg-slate-200 rounded-md"></div>
                  </div>
                  <div className="h-3 w-48 bg-slate-200/70 rounded"></div>
                </div>
              ))}
            </div>
          ) : units.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center py-10 px-4 text-center border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
              <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mb-3">
                <Wrench size={24} />
              </div>
              <h3 className="font-semibold text-slate-700">Belum Ada Unit AC Terdaftar</h3>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[420px] pr-1">
              {units.map(unit => (
                <div 
                  key={unit.id}
                  className="p-4 rounded-xl border border-slate-100 bg-slate-50 hover:bg-white hover:border-blue-200 transition-all shadow-xs flex flex-col sm:flex-row justify-between sm:items-center gap-3"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-800 text-sm">{unit.brand}</span>
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-xs">
            <span className="text-slate-500">Kebutuhan kustomisasi lebih lanjut?</span>
            <Link to="/pelanggan/unit" className="text-blue-600 font-semibold hover:underline">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col h-full">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-800">Status Permintaan Servis</h2>
                <p className="text-xs text-slate-500">Permintaan langsung terhubung ke sistem Admin</p>
              </div>
            </div>
            <button
              onClick={() => setIsRequestModalOpen(true)}
              className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 flex items-center gap-1 bg-indigo-50 px-2.5 py-1.5 rounded-lg transition-colors"
            >
          ...
          {isLoading ? (
            <div className="space-y-3 flex-1">
              {[1, 2].map(i => (
                <div key={i} className="p-4 rounded-xl border border-slate-100 bg-slate-50 animate-pulse space-y-2.5">
                  ...
                  <div className="h-3 w-56 bg-slate-200/70 rounded"></div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-10 px-4 text-center border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
              ...
              <h3 className="font-semibold text-slate-700">Belum Ada Permintaan Servis</h3>
              <p className="text-xs text-slate-500 max-w-xs mt-1 mb-4">
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[420px] pr-1">
              {requests.map(req => (
                <div 
                  key={req.id}
                  className="p-4 rounded-xl border border-slate-100 bg-white hover:border-indigo-200 transition-all shadow-xs flex flex-col gap-2"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-800 text-sm">{req.service}</span>
                        <span className="text-xs font-mono text-slate-400 font-semibold">{req.request_code}</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">
                        Jadwal: <span className="font-medium text-slate-700">{req.date}</span>
                  ...
                  {req.customer_notes && (
                    <p className="text-xs bg-slate-50 text-slate-600 p-2 rounded-lg italic">
                      "{req.customer_notes}"
                    </p>
                  )}
                  ...
                  {req.status === 'Selesai' && (req.before_photo_url || req.after_photo_url || req.technician_notes) && (
                    <div className="mt-2 p-3 bg-emerald-50/70 border border-emerald-100 rounded-xl space-y-2">
                      ...
                      {req.technician_notes && (
                        <p className="text-xs text-slate-700 bg-white/80 p-2 rounded border border-emerald-100/50">
                          {req.technician_notes}
                        </p>
                      )}
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        {req.before_photo_url && (
                          <div>
                            <span className="text-[10px] font-semibold text-slate-500 uppercase">Sebelum</span>
                            <img 
                              src={req.before_photo_url} 
                              alt="Kondisi Sebelum" 
                              className="w-full h-24 object-cover rounded-lg border border-slate-200 mt-0.5" 
                            />
                          </div>
                        )}
                        {req.after_photo_url && (
                          <div>
                            <span className="text-[10px] font-semibold text-emerald-600 uppercase">Sesudah Selesai</span>
                            <img 
                              src={req.after_photo_url} 
                              alt="Kondisi Sesudah" 
                              className="w-full h-24 object-cover rounded-lg border border-emerald-200 mt-0.5" 
                            />
                          </div>
                        )}
                      </div>
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {req.payment_status === 'Lunas' ? (
                              ...
                              <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-700 bg-amber-100/90 px-2.5 py-0.5 rounded-full">
                                <Clock size={13} /> Menunggu Verifikasi Admin
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 text-xs font-bold text-rose-700 bg-rose-100/90 px-2.5 py-0.5 rounded-full">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {req.payment_status === 'Menunggu Verifikasi' && (
                            <button
                              type="button"
                              onClick={() => handleOpenPaymentModal(req)}
                              className="px-3 py-1.5 bg-amber-50 border border-amber-300 text-amber-800 hover:bg-amber-100 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5"
                            >
                          ...
                          <Link 
                            ...
                            className="px-3 py-1.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 rounded-lg text-xs font-semibold flex items-center gap-1 transition-colors"
                          >
                            <FileText size={14} className="text-blue-600" /> Invoice
                          </Link>
                        </div>
                      </div>
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-xs">
            <span className="text-slate-500">Status pesanan diperbarui oleh Admin secara real-time.</span>
            <Link to="/pelanggan/pesan" className="text-indigo-600 font-semibold hover:underline">
      ...
      {isRequestModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-slate-100 animate-scaleUp">
            <div className="flex justify-between items-center pb-4 border-b border-slate-100">
              <div className="flex items-center gap-2 text-slate-800">
                <Calendar className="text-indigo-600" size={22} />
                <div>
                  <h3 className="text-lg font-bold">Kirim Permintaan Servis</h3>
                  <p className="text-xs text-slate-500">Permintaan akan langsung diterima oleh admin</p>
                </div>
              </div>
              <button 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {servicesList.map(s => (
                    ...
                    >
                      ...
                      <span className="text-[11px] text-slate-500">Rp {s.price.toLocaleString('id-ID')}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100 animate-scaleUp">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 sticky top-0 z-10">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-800">Unggah Bukti Transfer Bank</h3>
            ...
            <form onSubmit={handleSubmitPaymentProof} className="p-5 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  <div className="p-3 bg-blue-50/70 border border-blue-200 rounded-xl relative">
                    <span className="font-bold text-blue-900 block">Bank BCA</span>
                    <span className="font-mono text-sm font-bold text-slate-800 tracking-wide block mt-0.5">
                      8830-1234-5678
                    </span>
                    <span className="text-[10px] text-slate-500 block">a/n PT Laku AC Mandiri</span>
                    <button
                      type="button"
                      onClick={() => handleCopyAccount('883012345678', 'BCA')}
                      className="mt-2 text-[11px] font-semibold text-blue-700 hover:text-blue-900 flex items-center gap-1"
                    >
                      {copiedBank === 'BCA' ? (
                        <>
                  ...
                  <div className="p-3 bg-amber-50/70 border border-amber-200 rounded-xl relative">
                    <span className="font-bold text-amber-900 block">Bank Mandiri</span>
                    <span className="font-mono text-sm font-bold text-slate-800 tracking-wide block mt-0.5">
                      137-00-9876543-2
                    </span>
                    <span className="text-[10px] text-slate-500 block">a/n PT Laku AC Mandiri</span>
                    <button
                      ...
                      className="mt-2 text-[11px] font-semibold text-amber-800 hover:text-amber-950 flex items-center gap-1"
                    >
                      {copiedBank === 'Mandiri' ? (
                        <>
                          <Check size={12} className="text-emerald-600" /> Tersalin!
              ...
              <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-xs text-slate-500 block">Total Nominal Yang Harus Ditransfer:</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                    Bank Tujuan Transfer <span className="text-rose-500">*</span>
                  </label>
                  <select
                    value={paymentBank}
                    onChange={(e) => setPaymentBank(e.target.value)}
                    className="w-full px-3 py-2 text-xs border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 outline-none"
                  >
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                    Jumlah Ditransfer (Rp) <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="number"
                    required
                    ...
                    className="w-full px-3 py-2 text-xs border border-slate-300 rounded-xl font-semibold focus:ring-2 focus:ring-blue-500 outline-none"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Foto Bukti / Struk Transfer <span className="text-rose-500">*</span>
                </label>

                {proofImage ? (
                  <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-100 p-1">
                  ...
                  <div
                    ...
                    className="border-2 border-dashed border-slate-300 hover:border-blue-500 rounded-xl p-6 flex flex-col items-center justify-center text-slate-500 hover:bg-blue-50/30 transition-all cursor-pointer text-center group"
                  >
                    <Upload size={24} className="mb-1.5 text-slate-400 group-hover:text-blue-500 transition-colors" />
                    <span className="text-xs font-bold text-slate-700">Pilih / Foto Struk Bukti Transfer</span>
              ...
              <div className="pt-3 border-t border-slate-100 flex gap-3 sticky bottom-0 bg-white">
                <button
                  type="button"
                  onClick={() => setIsPaymentModalOpen(false)}
