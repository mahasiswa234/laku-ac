  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Riwayat Pekerjaan Selesai</h1>
          <p className="text-slate-600">Daftar pekerjaan servis yang telah diselesaikan beserta dokumentasi hasil.</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-slate-500 space-y-2">
            ...
            <p className="font-semibold text-slate-700">Belum ada riwayat pekerjaan yang selesai</p>
              ...
              <tbody className="text-sm divide-y divide-slate-100">
                {history.map((item, i) => (
                  <tr key={i} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {item.before_photo || item.after_photo ? (
                        <button
                          ...
                          className="flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg text-xs font-medium transition-colors border border-blue-100"
                        >
                          <Image size={14} />
                          Lihat Foto
                        </button>
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-semibold flex items-center gap-1.5 w-max">
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl animate-scaleUp">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Dokumentasi Hasil Kerja Teknisi</h3>
                <p className="text-xs text-slate-500">{selectedPhotoModal.id} - {selectedPhotoModal.customer}</p>
              </div>
              <button 
            ...
            <div className="py-4 space-y-4">
              {selectedPhotoModal.notes && (
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                  <p className="text-xs font-semibold text-slate-600 mb-1">Catatan Tindakan Teknisi:</p>
                  <p className="text-sm text-slate-800">{selectedPhotoModal.notes}</p>
                </div>
              )}

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-semibold text-slate-600 mb-1">Foto Sebelum Pengerjaan:</p>
                  {selectedPhotoModal.before_photo ? (
                    ...
                    <div className="w-full h-52 bg-slate-100 rounded-xl border border-dashed border-slate-200 flex items-center justify-center text-xs text-slate-400">
                      Tidak ada foto awal
                    </div>
                  )}
                </div>
                ...
                <div>
                  <p className="text-xs font-semibold text-emerald-700 mb-1">Foto Sesudah Selesai:</p>
                  {selectedPhotoModal.after_photo ? (
                    <img 
                      src={selectedPhotoModal.after_photo} 
                      alt="Sesudah" 
                      className="w-full h-52 object-cover rounded-xl border border-emerald-200" 
                    />
                  ) : (
                    <div className="w-full h-52 bg-slate-100 rounded-xl border border-dashed border-slate-200 flex items-center justify-center text-xs text-slate-400">
